...
export const getLoyaltyHistory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { history, count } = await loyaltyService.getLoyaltyHistory(
    userId.toString(),
    { page, limit, sort },
    req.id
  );
 ...
 * @access Protected
 */
export const getRedemptionById = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const userId = req.user._id;
    const { id } = req.params;
 ...
 * @access Protected (Admin)
 */
export const deleteLoyaltyTier = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const { id } = req.params;
...
export const getAllLoyaltyPrograms = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { programs, count } = await loyaltyService.getAllLoyaltyPrograms(
    filter,
    { page, limit, sort },
    req.id
  );
...
export const getAllRedemptions = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { redemptions, count } = await loyaltyService.getAllRedemptions(
    filter,
    { page, limit, sort },
    req.id
  );
 ...
 * @access Protected (Admin)
 */
export const updateRedemptionStatus = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const { id } = req.params;
