 ...
 */
export const getVendorDashboardSummary = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const dashboardSummary = await vendorDashboardService.getVendorDashboardSummary(
      vendorId.toString(),
      period,
      req.id
    );
 ...
 * @access Protected (Vendor)
 */
export const getVendorSalesAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const vendorId = req.vendor._id;
    ...
    if (interval && !["hourly", "daily", "weekly", "monthly"].includes(interval)) {
      return next(
        new ApiError("Invalid interval. Must be one of: hourly, daily, weekly, monthly", 400)
      );
    }
    ...
    if (groupBy && !["product", "category", "customer"].includes(groupBy)) {
      return next(
        new ApiError("Invalid groupBy. Must be one of: product, category, customer", 400)
      );
    }
 ...
 * @access Protected (Vendor)
 */
export const getVendorProductAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const vendorId = req.vendor._id;
 ...
 * @access Protected (Vendor)
 */
export const getVendorOrderAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const vendorId = req.vendor._id;
 ...
 * @access Protected (Vendor)
 */
export const getVendorPayoutAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    const vendorId = req.vendor._id;
 ...
 */
export const getVendorDashboardSummaryAdmin = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
