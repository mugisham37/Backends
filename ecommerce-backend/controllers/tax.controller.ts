 ...
 * @access Public
 */
export const getApplicableTaxRate = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
 ...
 * @access Public
 */
export const calculateTax = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const requestLogger = createRequestLogger(req.id);
    ...
    if (!amount || !country) {
      return next(
        new ApiError(translateError("taxCalculationParamsRequired", {}, req.language), 400)
      );
    }
