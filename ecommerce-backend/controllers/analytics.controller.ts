...
export const getSalesAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      groupBy &&
      !["product", "category", "vendor", "customer", "paymentMethod", "country"].includes(groupBy)
    ) {
      return next(
        new ApiError(
          "Invalid groupBy. Must be one of: product, category, vendor, customer, paymentMethod, country",
          400
        )
      );
    }
...
export const getCustomerAnalytics = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (segment && !["new", "returning", "inactive", "all"].includes(segment)) {
      return next(
        new ApiError("Invalid segment. Must be one of: new, returning, inactive, all", 400)
      );
    }
