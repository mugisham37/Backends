...
export const exportOrders = async (
  ...
  try {
    ...
    // Get orders
    const orders = await Order.find(query).populate("user", "email firstName lastName").sort("-createdAt").lean()
...
export const exportCustomers = async (
  ...
  try {
    ...
    const formattedCustomers = customers.map((customer) => {
      const orderData = customerOrderMap.get(customer._id.toString()) || { orderCount: 0, totalSpent: 0 }
      return {
        ...
        averageOrderValue: orderData.orderCount > 0 ? (orderData.totalSpent / orderData.orderCount).toFixed(2) : "0.00",
...
export const exportInventory = async (
  ...
  try {
    ...
    const formattedInventory = products.map((product) => {
      ...
      const variantQuantity = product.variants.reduce((total, variant) => total + variant.quantity, 0)
    ...
    if (filters.includeVariants === "true") {
        ...
        if (product.variants.length > 0) {
          product.variants.forEach((variant) => {
            variantDetails.push({
              ...
              sku: variant.sku,
              attributes: variant.attributes.map((attr) => `${attr.name}: ${attr.value}`).join(", "),
      ...
      if (variantDetails.length > 0) {
        const variantFields = ["productId", "productName", "sku", "attributes", "quantity", "price", "status"]
        ...
        switch (format) {
          ...
          case ExportFormat.EXCEL:
            return exportToExcel(variantDetails, variantFields, ExportDataType.INVENTORY, requestId)
          case ExportFormat.PDF:
            return exportToPdf(
              variantDetails,
              variantFields,
