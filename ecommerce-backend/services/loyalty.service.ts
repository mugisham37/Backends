 ...
 */
export const getCustomerLoyaltyProgram = async (
  userId: string,
  requestId?: string
): Promise<any> => {
  ...
  try {
    ...
    const pointsForNextTier = nextTier
      ? nextTier.pointsThreshold - loyaltyProgram.lifetimePoints
      : null;
 ...
 */
export const getCustomerLoyaltyDashboard = async (
  userId: string,
  requestId?: string
): Promise<any> => {
  const logger = createRequestLogger(requestId);
  ...
  try {
    ...
    const [loyaltyProgram, recentHistory, activeRedemptions, availableRewards, referralStats] =
      await Promise.all([
        // Get loyalty program
...
export const getLoyaltyStatisticsByPeriod = async (
  ...
  try {
    ...
    const totalPoints = pointsByType.reduce(
      (sum, item) => sum + (item.totalPoints > 0 ? item.totalPoints : 0),
      0
    );
