...
export const getAllCountries = async (requestId?: string): Promise<ICountry[]> => {
  ...
  try {
    const countries = await Country.find({ isActive: true }).sort({ name: 1 }).populate("currency").lean()
...
export const getCountryByCode = async (code: string, requestId?: string): Promise<ICountry> => {
  ...
  try {
    const country = await Country.findOne({ code: code.toUpperCase(), isActive: true }).populate("currency").lean()
 ...
 */
export const createCountry = async (countryData: Partial<ICountry>, requestId?: string): Promise<ICountry> => {
  const logger = createRequestLogger(requestId)
...
export const getStatesByCountry = async (
  ...
  try {
    const country = await Country.findOne({ code: countryCode.toUpperCase(), isActive: true }).select("states").lean()
