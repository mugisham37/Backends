...
export const getAllTaxRates = async (requestId?: string): Promise<ITaxRate[]> => {
  ...
  try {
    const taxRates = await TaxRate.find({ isActive: true }).sort({ country: 1, priority: -1 }).lean()
 ...
 */
export const createTaxRate = async (taxRateData: Partial<ITaxRate>, requestId?: string): Promise<ITaxRate> => {
  const logger = createRequestLogger(requestId)
