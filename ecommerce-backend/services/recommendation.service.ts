...
export const getPersonalizedRecommendations = async (
  ...
  try {
    ...
    const orderedProductIds = userOrders.flatMap((order) => order.orderItems.map((item) => item.product.toString()))
    ...
    if (recommendedProducts.length < limit) {
      ...
      const existingProductIds = [...recommendedProducts.map((product) => product._id), ...uniqueOrderedProductIds]
...
export const getFrequentlyBoughtTogether = async (productId: string, limit = 3, requestId?: string): Promise<any[]> => {
  ...
  try {
    ...
    if (sortedProducts.length < limit) {
      ...
      const existingProductIds = [...sortedProducts.map((product) => product._id.toString()), productId]
