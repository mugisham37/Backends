 ...
 * Index a document
 */
export const indexDocument = async <T extends { id: string }>(index: string, document: T): Promise<void> => {
 ...
 */
export const bulkIndexDocuments = async <T extends { id: string }>(index: string, documents: T[]): Promise<void> => {
  if (!config.elasticsearch.enabled || !esClient || documents.length === 0) return
