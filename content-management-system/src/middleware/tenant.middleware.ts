...
export const resolveTenant = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const tenantId = req.headers["x-tenant-id"] || req.query.tenantId || req.params.tenantId || req.body.tenantId
 ...
 */
export const checkTenantMembership = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!(req as any).tenant) {
      return next(ApiError.badRequest("Tenant ID is required"))
    }
 ...
 */
export const trackTenantApiRequest = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if ((req as any).tenant) {
      const tenantId = (req as any).tenant._id
...
export const checkTenantLimit = (limitType: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (hasReachedLimit) {
        return next(ApiError.forbidden(`Tenant has reached the ${limitType} limit (${currentUsage}/${limit})`))
      }
